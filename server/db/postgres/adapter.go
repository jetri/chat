...
const (
	defaultDSN      = "host=postgres user=postgres password=angels03! dbname=tinode sslmode=disable"
...
func (a *adapter) GetDbVersion() (int, error) {
	...
	var vers int
	err := a.db.Get(&vers, "SELECT value FROM kvmeta WHERE key=$1", "version")
	if err != nil {
...
func (a *adapter) CreateDb(reset bool) error {
	...
	if _, err = tx.Exec(
		`CREATE TABLE usertags(
			...
			CONSTRAINT fx_usertags_userid_id FOREIGN KEY(userid) REFERENCES users(id) INITIALLY DEFERRED,
	...
	if _, err = tx.Exec(
		`CREATE TABLE devices(
			...
			lang     VARCHAR(8),
			CONSTRAINT fk_devices_userid_id FOREIGN KEY(userid) REFERENCES users(id),
	...
	if _, err = tx.Exec(
		`CREATE TABLE auth(
			...
			expires TIMESTAMP,
			CONSTRAINT fk_auth_userid_id FOREIGN KEY(userid) REFERENCES users(id),
	...
	if _, err = tx.Exec(
		`CREATE TABLE topictags(
			...
			tag   VARCHAR(96) NOT NULL,
			CONSTRAINT fk_topictags_topic_name FOREIGN KEY(topic) REFERENCES topics(name) INITIALLY DEFERRED,
	...
	if _, err = tx.Exec(
		`CREATE TABLE subscriptions(
			...
			modegiven BYTEA,
			private   JSONB,
			CONSTRAINT fk_subscriptions_userid_id FOREIGN KEY(userid) REFERENCES users(id),
	...
	if _, err = tx.Exec(
		`CREATE TABLE messages(
			...
			"head"     JSONB,
			content   JSONB,
			CONSTRAINT fk_messages_topic_name FOREIGN KEY(topic) REFERENCES topics(name),
	...
	if _, err = tx.Exec(
		`CREATE TABLE dellog(
			...
			low        INT NOT NULL,
			hi         INT NOT NULL,
			CONSTRAINT fk_dellog_topic_name FOREIGN KEY(topic) REFERENCES topics(name)
		);
	...
	if _, err = tx.Exec(
		`CREATE TABLE credentials(
			...
			CONSTRAINT fk_credentials_userid_id FOREIGN KEY(userid) REFERENCES users(id)
		);`); err != nil {
		return err
	}
	...
	if _, err = tx.Exec(
		`CREATE TABLE filemsglinks(
			...
			msgid 		INT NOT NULL,
			CONSTRAINT fk_filemsglinks_fileid_id FOREIGN KEY(fileid) REFERENCES fileuploads(id) ON DELETE CASCADE,
			CONSTRAINT fk_filemsglinks_msgid_id FOREIGN KEY(msgid) REFERENCES messages(id) ON DELETE CASCADE
		)`); err != nil {
		return err
	}
	...
	if _, err = tx.Exec("INSERT INTO kvmeta(key, value) VALUES('version', $1)", strconv.Itoa(adpVersion)); err != nil {
		return err
	}
...
func (a *adapter) UserUpdate(uid t.Uid, update map[string]interface{}) error {
	...
	q := "UPDATE users SET " + strings.Join(cols, ",") + " WHERE id=?"
	q = sqlx.Rebind(sqlx.DOLLAR, q)
...
func (a *adapter) UserUnreadCount(uid t.Uid) (int, error) {
	...
	err := a.db.Get(&count, "SELECT SUM(t.seqid)-SUM(s.readseqid) FROM topics AS t, subscriptions AS s "+
		...
		"POSITION('R' IN s.modewant)>0 AND POSITION('R' IN s.modegiven)>0", store.DecodeUid(uid), t.StateDeleted)
	if err == nil {
		return count, nil
	}
...
func (a *adapter) TopicsForUser(uid t.Uid, keepDeleted bool, opts *t.QueryOpt) ([]t.Subscription, error) {
	...
	if err == nil && len(usrq) > 0 {
		q, usrq, _ := sqlx.In(
			"SELECT id, createdat,updatedat,state,stateat,access,lastseen,useragent,public,tags "+
...
func (a *adapter) TopicUpdate(topic string, update map[string]interface{}) error {
	...
	q := "UPDATE topics SET " + strings.Join(cols, ",") + " WHERE name=?"
	q = sqlx.Rebind(sqlx.DOLLAR, q)
	_, err = tx.Exec(q, args...)
	if err != nil {
		return err
	...
	if tags := extractTags(update); tags != nil {
		// First delete all user tags
		_, err = tx.Exec("DELETE FROM topictags WHERE topic=$2", topic)
		if err != nil {
			return err
		}
		// Now insert new tags
...
func messageDeleteList(tx *sqlx.Tx, topic string, toDel *t.DelMessage) error {
	...
	if toDel == nil {
		...
		var insert *sql.Stmt
		if insert, err = tx.Prepare("INSERT INTO dellog(topic,deletedfor,delid,low,hi) VALUES($1,$2,$3,$4,$5)"); err != nil {
			return err
		}
		...
		if err == nil && toDel.DeletedFor == "" {
			...
			d := "DELETE fml.* FROM filemsglinks AS fml INNER JOIN messages AS m ON m.id=fml.msgid WHERE "
			d = sqlx.Rebind(sqlx.DOLLAR, d)
			_, err = tx.Exec(d+
				where, args...)
			if err != nil {
				return err
			...
			u := "UPDATE messages AS m SET m.deletedAt=?,m.delId=?,m.head=NULL,m.content=NULL WHERE "
			u = sqlx.Rebind(sqlx.DOLLAR, u)
			_, err = tx.Exec(u+
				where,
...
func (a *adapter) MessageAttachments(msgId t.Uid, fids []string) error {
	...
	q := sqlx.Rebind(sqlx.DOLLAR, "INSERT INTO filemsglinks(createdat,fileid,msgid) VALUES "+strings.Join(values, ","))
	_, err = a.db.Exec(q, args...)
	if err != nil {
		return err
	}

	u := sqlx.Rebind(sqlx.DOLLAR, "UPDATE fileuploads SET updatedat='"+strNow+"' WHERE id IN (?"+
		strings.Repeat(",?", len(args)-1)+")")
	_, err = tx.Exec(u, args...)
	if err != nil {
		return err
	}
...
func credDel(tx *sqlx.Tx, uid t.Uid, method, value string) error {
	...
	q := "DELETE FROM credentials" + constraints
	q = sqlx.Rebind(sqlx.DOLLAR, q)
	...
	// Case 2.1
	res, err = tx.Exec(q+" AND (done=true OR retries=0)", args...)
	if err != nil {
		return err
	}
	if count, _ := res.RowsAffected(); count > 0 {
	...
	u := "UPDATE credentials SET deletedat=?" + constraints
	u = sqlx.Rebind(sqlx.DOLLAR, u)
	res, err = tx.Exec(u, args...)
	if err == nil {
		if count, _ := res.RowsAffected(); count >= 0 {
			err = t.ErrNotFound
		}
